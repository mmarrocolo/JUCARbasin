...
def Weigfun( Tlag): 
	...
	if nmax==1: 
		...
		Weigths=np.zeros(int(nmax)) # here int
		...
		for i in range(nh+1,int(np.floor(Tlag))): # here int
			Weigths[i]=(Tlag-float(i+1)+.5)/th

		if Tlag>np.floor(Tlag): # here int
			Weigths[int(np.floor(Tlag))]=(Tlag-np.floor(Tlag))**2/(2*th) # here int
