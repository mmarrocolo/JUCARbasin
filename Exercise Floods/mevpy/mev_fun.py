...
###############################################################################
        ...
        years with just a few observations should not be used
        (e.g., I select years with less than 10% of missing values)
###############################################################################
'''
# import sys
import numpy as np
...
def mev_fit(df, ws = 1, how = 'pwm', threshold = 1, potmode = False, declu = False):
    '''--------------------------------------------------------------------
    ...
    threshold: threshold for defining ordinary events (default is one)
...
def mev_CI(df, Fi_val, x0, ws = 1, ntimes = 1000, MEV_how = 'pwm', 
    ...
    potmode: if True, fit WEI to excesses above threshold
             if False, fit WEI to entire values, neglecting observations below threshold (default)
    declu: If True, before fitting WEI decluster time series so that only indep events are used for fitting MEV
    (computed globally for the whole time series)
...
def decluster(sample, noise_lag = 10, noise_prob = 0.9, max_lag = 30):
    ...
    :param noise_prob: assume as noise correlation the 90% percentile of values beyond noise_lag
...
def table_rainfall_maxima(df, how = 'pwm', thresh = 0, potmode = True, declu = False):
    ...
    potmode: if True, fit WEI to excesses above threshold
             if False, fit WEI to entire values, neglecting observations below threshold (default is False)
     declu: If True, before fitting WEI decluster time series so that only indep events are used for fitting MEV
    (computed globally for the whole time series)
    returns:
    XI -> array of annual maxima (ranked in ascending order)
